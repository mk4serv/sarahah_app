...
export const LoginServices = async (req, res) => {
    try {
        ...
        // ✅ Generate JWT
        const accesstoken = jwt.sign({ _id:user._id, email:user.email }, process.env.JWT_SECRET_LOGIN_KEY, { expiresIn: '1h' });

        return res.status(200).json({ message: 'Logged in successfully', isPasswordCorrect, accesstoken });
